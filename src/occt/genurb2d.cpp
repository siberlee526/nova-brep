// genurb2d.cpp - implementation of NvGeNurbCurve2d based on OCCT Geom2d_BSplineCurve.
//
...
// file-local Geom2d_BSplineCurve subclass (NvGeNurb2dData); every OCCT
...
// to its multiplicity - exactly like Geom2d_BSplineCurve::KnotSequence().
...
// Side data riding on NvGeNurb2dData: the interpolation ("fit") definition
// and the evaluation-mode flag. Copied along with every private copy.
struct NvGeFitData2d
{
  NvGePoint2dArray            Points;                   // fit points
  std::vector<double>         Params;                   // explicit parameters; empty = compute
  NvGeVector2dArray           Tangents;                 // per-point tangents (TangentsExist)
...
class NvGeNurb2dData : public Geom2d_BSplineCurve
{
public:

  NvGeNurb2dData (const NCollection_Array1<gp_Pnt2d>& thePoles,
  ...
  NvGeNurb2dData (const NCollection_Array1<gp_Pnt2d>& thePoles,
                  ...
                  const int theDegree,
                  const bool thePeriodic)
  : Geom2d_BSplineCurve (thePoles, theWeights, theKnots, theMults, theDegree, thePeriodic)
  {
  }

  //! Definition copy without side data.
  explicit NvGeNurb2dData (const Geom2d_BSplineCurve& theSrc)
  : Geom2d_BSplineCurve (theSrc)
  {
  }

  //! Definition copy carrying fit side data (used after interpolation).
  NvGeNurb2dData (const Geom2d_BSplineCurve& theSrc, const NvGeFitData2d& theFit)
  : Geom2d_BSplineCurve (theSrc),
    Fit (theFit)
  {
  }
  ...
  NvGeNurb2dData (const NvGeNurb2dData& theSrc)
  : Geom2d_BSplineCurve (theSrc),
    Fit (theSrc.Fit)
  {
  }
...
occ::handle<Geom2d_BSplineCurve> SplineOf (const NvGeImpEntity3d* theImp)
{
  ...
  if (aSpline.IsNull())
  {
    throw NvException ("NvGeNurbCurve2d: the entity does not hold a B-spline curve");
  }
  return aSpline;
}

// The side data behind theSpline; null when the spline carries none.
const NvGeNurb2dData* FitDataOf (const occ::handle<Geom2d_BSplineCurve>& theSpline)
{
  return dynamic_cast<const NvGeNurb2dData*> (theSpline.get());
}
...
double KnotToleranceOf (const Geom2d_BSplineCurve& theSpline)
{
...
NvGeKnotVector FlatKnotVector (const Geom2d_BSplineCurve& theSpline)
{
...
Definition DefinitionOf (const Geom2d_BSplineCurve& theSpline)
{
  Definition aDef;
  aDef.Degree = theSpline.Degree();
  aDef.Periodic = theSpline.IsPeriodic();
  aDef.Rational = theSpline.Weights() != nullptr;
  const NCollection_Array1<gp_Pnt2d>& aPoles = theSpline.Poles();
...
occ::handle<NvGeNurb2dData> DataFromDefinition (const Definition& theDef, const char* theMethod)
{
  const std::string aCtx = std::string ("NvGeNurbCurve2d::") + theMethod + "(): ";
  if (theDef.Degree < 1 || theDef.Degree > Geom2d_BSplineCurve::MaxDegree())
  {
  ...
  NCollection_Array1<gp_Pnt2d> aPoles (1, static_cast<int> (theDef.Poles.size()));
  ...
  try
  {
    ...
    {
      return occ::handle<NvGeNurb2dData> (new NvGeNurb2dData (aPoles, aKnots, aMults, theDef.Degree, theDef.Periodic));
    }
    ...
    return occ::handle<NvGeNurb2dData> (new NvGeNurb2dData (aPoles, aWeights, aKnots, aMults, theDef.Degree, theDef.Periodic));
  }
...
std::vector<double> ComputeParams (const NvGePoint2dArray& thePoints,
...
occ::handle<NvGeNurb2dData> InterpolateFit (const NvGeFitData2d& theFit, const char* theMethod)
{
  const std::string aCtx = std::string ("NvGeNurbCurve2d::") + theMethod + "(): ";
  ...
  std::vector<gp_Pnt2d> aPnts;
  aPnts.reserve (aCount);
  for (int i = 0; i < aCount; ++i)
  {
    aPnts.push_back (gp_Pnt2d (theFit.Points[i].x, theFit.Points[i].y));
  }
  ...
  occ::handle<NCollection_HArray1<gp_Pnt2d>> aPoints = new NCollection_HArray1<gp_Pnt2d> (1, aUseCount);
  ...
  Geom2dAPI_Interpolate anInterp (aPoints, aParameters, theFit.Periodic, theFit.Tolerance.equalPoint());

  // Tangency constraints: per-point tangents when present, otherwise the
  // end tangents through the tangent-defined flags.
  NCollection_Array1<gp_Vec2d> aTangents (1, aUseCount);
  ...
  if (theFit.TangentsExist)
  {
    for (int i = 0; i < aUseCount; ++i)
    {
      aTangents.SetValue (i + 1, gp_Vec2d (theFit.Tangents[i].x, theFit.Tangents[i].y));
  ...
  {
    aTangents.SetValue (1, gp_Vec2d (theFit.StartTangent.x, theFit.StartTangent.y));
    aTangents.SetValue (aUseCount, gp_Vec2d (theFit.EndTangent.x, theFit.EndTangent.y));
  ...
  return occ::handle<NvGeNurb2dData> (new NvGeNurb2dData (*anInterp.Curve(), theFit));
}
...
// middle weights.
occ::handle<NvGeNurb2dData> EllipseSpline (const NvGeEllipArc2d& theEllipse)
{
  const NvGePoint2d aCenter = theEllipse.center();
  NvGeVector2d anXAxis = theEllipse.majorAxis();
  NvGeVector2d aYAxis = theEllipse.minorAxis();
  const double aMajorR = theEllipse.majorRadius();
  const double aMinorR = theEllipse.minorRadius();
  double aStart = theEllipse.startAng();
  double anEnd = theEllipse.endAng();
  if (anXAxis.isZeroLength() || aYAxis.isZeroLength() || aMajorR <= 0.0 || aMinorR <= 0.0)
  {
    throw NvException ("NvGeNurbCurve2d::NvGeNurbCurve2d(): the ellipse is degenerate");
  }
  if (theEllipse.isClockWise())
  {
    // Mirror into the counterclockwise frame.
    aYAxis.negate();
  ...
  {
    return gp_Pnt2d (aCenter.x + (aMajorR * std::cos (theAng)) * anXAxis.x + (aMinorR * std::sin (theAng)) * aYAxis.x,
  ...
  {
    return gp_Pnt2d (aCenter.x + (aMajorR * std::cos (theAng)) * anXAxis.x * aMidScale + (aMinorR * std::sin (theAng)) * aYAxis.x * aMidScale,
  ...
  return DataFromDefinition (aDef, "NvGeNurbCurve2d");
}
...
occ::handle<NvGeNurb2dData> ClosedCopy (const NvGeNurb2dData& theSrc, const char* theMethod)
{
  if (theSrc.EndPoint().Distance (theSrc.StartPoint()) <= NvGeContext::gTol.equalPoint())
  {
    return occ::handle<NvGeNurb2dData> (new NvGeNurb2dData (theSrc));
  }
  Definition aDef = DefinitionOf (theSrc);
  if (aDef.Mults.front() != aDef.Degree + 1 || aDef.Mults.back() != aDef.Degree + 1)
  {
    throw NvException (std::string ("NvGeNurbCurve2d::") + theMethod + "(): a clamped spline is required");
  }
  ...
  occ::handle<NvGeNurb2dData> aRes = DataFromDefinition (aDef, theMethod);
  aRes->Fit = theSrc.Fit;
  return aRes;
}
...
occ::handle<NvGeNurb2dData> PrivateCopy (const occ::handle<Geom2d_BSplineCurve>& theSpline)
{
  const NvGeNurb2dData* aSrc = FitDataOf (theSpline);
  if (aSrc != nullptr)
  {
    return occ::handle<NvGeNurb2dData> (new NvGeNurb2dData (*aSrc));
  }
  return occ::handle<NvGeNurb2dData> (new NvGeNurb2dData (*theSpline));
}
...
NvGeNurbCurve2d::NvGeNurbCurve2d ()
{
  ...
  aMults.SetValue (1, 2);
  aMults.SetValue (2, 2);
  mpImpEnt = new NvGeImpEntity3d (NvGe::kNurbCurve2d,
                                  occ::handle<Geom2d_BSplineCurve> (new NvGeNurb2dData (aPoles, aKnots, aMults, 1, false)));
  mpImpEnt->Ref();
}

//=================================================================================================

NvGeNurbCurve2d::NvGeNurbCurve2d (const NvGeNurbCurve2d& theSrc)
...
NvGeNurbCurve2d::NvGeNurbCurve2d (int theDegree, const NvGeKnotVector& theKnots,
                                  const NvGePoint2dArray& theCntrlPnts, Adesk::Boolean theIsPeriodic)
{
  if (mDelEnt && mpImpEnt != nullptr)
  {
    mpImpEnt->Unref();
  ...
  {
    aDef.Poles.push_back (gp_Pnt2d (theCntrlPnts[i].x, theCntrlPnts[i].y));
  }
  const KnotStructure aKnots = KnotsAndMults (theKnots);
  aDef.Knots = aKnots.Knots;
  aDef.Mults = aKnots.Mults;
  mpImpEnt = new NvGeImpEntity3d (NvGe::kNurbCurve2d, DataFromDefinition (aDef, "NvGeNurbCurve2d"));
  mpImpEnt->Ref();
}
...
NvGeNurbCurve2d::NvGeNurbCurve2d (int theDegree, const NvGeKnotVector& theKnots,
                                  const NvGePoint2dArray& theCntrlPnts,
...
NvGeNurbCurve2d::NvGeNurbCurve2d (int theDegree, const NvGePolyline2d& theFitPolyline,
                                  Adesk::Boolean theIsPeriodic)
{
  ...
  if (theDegree < 2)
  {
    throw NvException ("NvGeNurbCurve2d::NvGeNurbCurve2d(): the degree must be at least 2");
  }
  NvGeFitData2d aFit;
  ...
  occ::handle<NvGeNurb2dData> aData = InterpolateFit (aFit, "NvGeNurbCurve2d");
  if (theDegree > 3)
  {
    try
    {
  ...
  mpImpEnt = new NvGeImpEntity3d (NvGe::kNurbCurve2d, aData);
  mpImpEnt->Ref();
}

//=================================================================================================

NvGeNurbCurve2d::NvGeNurbCurve2d (const NvGePoint2dArray& theFitPoints,
                                  const NvGeVector2d& theStartTangent,
                                  const NvGeVector2d& theEndTangent,
                                  ...
                                  const NvGeTol& theFitTolerance)
{
  ...
  mpImpEnt = new NvGeImpEntity3d (NvGe::kNurbCurve2d, InterpolateFit (aFit, "NvGeNurbCurve2d"));
  mpImpEnt->Ref();
}
                                  ...
                                  const NvGeTol& theFitTolerance)
{
  ...
  {
    throw NvException ("NvGeNurbCurve2d::NvGeNurbCurve2d(): the knot parameterization is not defined");
  }
  NvGeFitData2d aFit;
  aFit.Points = theFitPoints;
  aFit.EndTangentsExist = true;
...
NvGeNurbCurve2d::NvGeNurbCurve2d (const NvGePoint2dArray& theFitPoints, const NvGeTol& theFitTolerance)
{
  if (mDelEnt && mpImpEnt != nullptr)
  {
    mpImpEnt->Unref();
...
NvGeNurbCurve2d::NvGeNurbCurve2d (const NvGePoint2dArray& theFitPoints,
                                  const NvGeVector2dArray& theFitTangents,
                                  const NvGeTol& theFitTolerance,
                                  Adesk::Boolean theIsPeriodic)
{
  if (mDelEnt && mpImpEnt != nullptr)
...
NvGeNurbCurve2d::NvGeNurbCurve2d (const NvGeEllipArc2d& theEllipse)
{
  if (mDelEnt && mpImpEnt != nullptr)
  {
    mpImpEnt->Unref();
  }
  mpImpEnt = new NvGeImpEntity3d (NvGe::kNurbCurve2d, EllipseSpline (theEllipse));
  mpImpEnt->Ref();
}

//=================================================================================================

NvGeNurbCurve2d::NvGeNurbCurve2d (const NvGeLineSeg2d& theLinSeg)
{
  if (mDelEnt && mpImpEnt != nullptr)
  {
    mpImpEnt->Unref();
  }
  const NvGePoint2d aStart = theLinSeg.startPoint();
  const NvGePoint2d anEnd = theLinSeg.endPoint();
  Definition aDef;
  aDef.Degree = 1;
  aDef.Poles.push_back (gp_Pnt2d (aStart.x, aStart.y));
  aDef.Poles.push_back (gp_Pnt2d (anEnd.x, anEnd.y));
...
NvGeNurbCurve2d::NvGeNurbCurve2d (const NvGeCurve2d& theCurve, double theEpsilon)
{
  if (mDelEnt && mpImpEnt != nullptr)
  {
    mpImpEnt->Unref();
  ...
  if (theEpsilon <= 0.0)
  {
    throw NvException ("NvGeNurbCurve2d::NvGeNurbCurve2d(): the approximation tolerance must be positive");
  }
  NvGeInterval anInterval;
  theCurve.getInterval (anInterval);
  if (!anInterval.isBounded())
  {
    throw NvException ("NvGeNurbCurve2d::NvGeNurbCurve2d(): the curve has no bounded interval");
  }
  NvGeFitData2d aFit;
...
int NvGeNurbCurve2d::numFitPoints () const
{
  const NvGeNurb2dData* aData = FitDataOf (SplineOf (mpImpEnt));
  return aData != nullptr ? aData->Fit.Points.length() : 0;
}

//=================================================================================================

Adesk::Boolean NvGeNurbCurve2d::getFitPointAt (int theIndex, NvGePoint2d& thePoint) const
{
  const NvGeNurb2dData* aData = FitDataOf (SplineOf (mpImpEnt));
...
Adesk::Boolean NvGeNurbCurve2d::getFitTolerance (NvGeTol& theFitTolerance) const
{
  const NvGeNurb2dData* aData = FitDataOf (SplineOf (mpImpEnt));
  if (aData == nullptr)
  {
...
Adesk::Boolean NvGeNurbCurve2d::getFitTangents (NvGeVector2d& theStartTangent,
                                                NvGeVector2d& theEndTangent) const
{
  const NvGeNurb2dData* aData = FitDataOf (SplineOf (mpImpEnt));
...
Adesk::Boolean NvGeNurbCurve2d::getFitKnotParameterization (KnotParameterization& theKnotParam) const
{
  const NvGeNurb2dData* aData = FitDataOf (SplineOf (mpImpEnt));
  if (aData == nullptr)
  {
...
Adesk::Boolean NvGeNurbCurve2d::getFitData (NvGePoint2dArray& theFitPoints,
                                            NvGeTol& theFitTolerance,
                                            Adesk::Boolean& theTangentsExist,
                                            NvGeVector2d& theStartTangent,
                                            NvGeVector2d& theEndTangent) const
{
  const NvGeNurb2dData* aData = FitDataOf (SplineOf (mpImpEnt));
  if (aData == nullptr)
...
void NvGeNurbCurve2d::getDefinitionData (int& theDegree, Adesk::Boolean& theRational,
                                         Adesk::Boolean& thePeriodic, NvGeKnotVector& theKnots,
                                         NvGePoint2dArray& theControlPoints,
                                         NvGeDoubleArray& theWeights) const
{
  ...
  const NCollection_Array1<gp_Pnt2d>& aPoles = aSpline->Poles();
  for (int i = aPoles.Lower(); i <= aPoles.Upper(); ++i)
  {
    theControlPoints.append (NvGePoint2d (aPoles.Value (i).X(), aPoles.Value (i).Y()));
  }
...
int NvGeNurbCurve2d::numWeights () const
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
...
double NvGeNurbCurve2d::weightAt (int theIdx) const
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (theIdx < 0 || theIdx >= aSpline->NbPoles())
  {
    throw NvException ("NvGeNurbCurve2d::weightAt(): the index is out of range");
  }
...
Adesk::Boolean NvGeNurbCurve2d::evalMode () const
{
  const NvGeNurb2dData* aData = FitDataOf (SplineOf (mpImpEnt));
...
Adesk::Boolean NvGeNurbCurve2d::getParamsOfC1Discontinuity (NvGeDoubleArray& theParams,
                                                            const NvGeTol&) const
{
  theParams.removeAll();
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
...
Adesk::Boolean NvGeNurbCurve2d::getParamsOfG1Discontinuity (NvGeDoubleArray& theParams,
...
Adesk::Boolean NvGeNurbCurve2d::setFitPointAt (int theIndex, const NvGePoint2d& thePoint)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  const NvGeNurb2dData* aSrc = FitDataOf (aSpline);
  if (aSrc == nullptr || aSrc->Fit.Points.length() == 0
      || theIndex < 0 || theIndex >= aSrc->Fit.Points.length())
  {
    return Adesk::kFalse;
  }
  occ::handle<NvGeNurb2dData> aData = PrivateCopy (aSpline);
...
Adesk::Boolean NvGeNurbCurve2d::addFitPointAt (int theIndex, const NvGePoint2d& thePoint)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  const NvGeNurb2dData* aSrc = FitDataOf (aSpline);
...
Adesk::Boolean NvGeNurbCurve2d::deleteFitPointAt (int theIndex)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  const NvGeNurb2dData* aSrc = FitDataOf (aSpline);
  if (aSrc == nullptr || aSrc->Fit.Periodic
...
Adesk::Boolean NvGeNurbCurve2d::setFitTolerance (const NvGeTol& theFitTol)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
...
Adesk::Boolean NvGeNurbCurve2d::setFitTangents (const NvGeVector2d& theStartTangent,
                                                const NvGeVector2d& theEndTangent)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  const NvGeNurb2dData* aSrc = FitDataOf (aSpline);
...
Adesk::Boolean NvGeNurbCurve2d::setFitKnotParameterization (KnotParameterization theKnotParam)
{
...
NvGeNurbCurve2d& NvGeNurbCurve2d::setFitData (const NvGePoint2dArray& theFitPoints,
                                              const NvGeVector2d& theStartTangent,
                                              const NvGeVector2d& theEndTangent,
                                              ...
                                              const NvGeTol& theFitTol)
{
  if (theKnotParam == NvGe::kNotDefinedKnotParam)
  {
    throw NvException ("NvGeNurbCurve2d::setFitData(): the knot parameterization is not defined");
  }
  NvGeFitData2d aFit;
  aFit.Points = theFitPoints;
  aFit.EndTangentsExist = true;
...
NvGeNurbCurve2d& NvGeNurbCurve2d::setFitData (const NvGeKnotVector& theFitKnots,
                                              ...
                                              Adesk::Boolean theIsPeriodic)
{
  ...
  {
    throw NvException ("NvGeNurbCurve2d::setFitData(): the fit knot count does not match the"
  ...
  for (int i = 1; i < aNeed; ++i)
  {
    ...
    {
      throw NvException ("NvGeNurbCurve2d::setFitData(): the fit knots must be strictly increasing");
    }
  }
...
NvGeNurbCurve2d& NvGeNurbCurve2d::setFitData (int theDegree,
                                              const NvGePoint2dArray& theFitPoints,
                                              const NvGeTol& theFitTol)
{
  if (theDegree < 2)
  {
    throw NvException ("NvGeNurbCurve2d::setFitData(): the degree must be at least 2");
  }
  if (theDegree > Geom2d_BSplineCurve::MaxDegree())
  {
    throw NvException ("NvGeNurbCurve2d::setFitData(): the degree is out of range");
  }
  NvGeFitData2d aFit;
  aFit.Points = theFitPoints;
  aFit.Tolerance = theFitTol;
  occ::handle<NvGeNurb2dData> aData = InterpolateFit (aFit, "setFitData");
  if (theDegree > 3)
  {
    try
    {
...
Adesk::Boolean NvGeNurbCurve2d::purgeFitData ()
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (FitDataOf (aSpline) == nullptr)
  {
  ...
  // Copy() yields a plain Geom2d_BSplineCurve without side data.
  ReplaceGeometry (mpImpEnt, occ::down_cast<Geom2d_BSplineCurve> (aSpline->Copy()));
  return Adesk::kTrue;
}

//=================================================================================================

Adesk::Boolean NvGeNurbCurve2d::buildFitData ()
{
...
Adesk::Boolean NvGeNurbCurve2d::buildFitData (KnotParameterization theKp)
{
...
NvGeNurbCurve2d& NvGeNurbCurve2d::addKnot (double theNewKnot)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (theNewKnot < aSpline->FirstParameter() || theNewKnot > aSpline->LastParameter())
  {
    throw NvException ("NvGeNurbCurve2d::addKnot(): the knot is outside the parameter range");
  }
  occ::handle<NvGeNurb2dData> aData = PrivateCopy (aSpline);
  try
  {
...
NvGeNurbCurve2d& NvGeNurbCurve2d::insertKnot (double theNewKnot)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (theNewKnot < aSpline->FirstParameter() || theNewKnot > aSpline->LastParameter())
  {
    throw NvException ("NvGeNurbCurve2d::insertKnot(): the knot is outside the parameter range");
  }
...
NvGeSplineEnt2d& NvGeNurbCurve2d::setWeightAt (int theIdx, double theVal)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (theVal <= 0.0)
  {
    throw NvException ("NvGeNurbCurve2d::setWeightAt(): the weight must be positive");
  }
  if (theIdx < 0 || theIdx >= aSpline->NbPoles())
  {
    throw NvException ("NvGeNurbCurve2d::setWeightAt(): the index is out of range");
  }
  occ::handle<NvGeNurb2dData> aData = PrivateCopy (aSpline);
  try
  {
...
NvGeNurbCurve2d& NvGeNurbCurve2d::setEvalMode (Adesk::Boolean theEvalMode)
{
  occ::handle<NvGeNurb2dData> aData = PrivateCopy (SplineOf (mpImpEnt));
...
NvGeNurbCurve2d& NvGeNurbCurve2d::joinWith (const NvGeNurbCurve2d& theCurve)
{
  const occ::handle<Geom2d_BSplineCurve> aLeft = SplineOf (mpImpEnt);
  const occ::handle<Geom2d_BSplineCurve> aRight = SplineOf (theCurve.mpImpEnt);
  if (aLeft->IsPeriodic() || aRight->IsPeriodic())
  {
    throw NvException ("NvGeNurbCurve2d::joinWith(): a periodic spline cannot be joined");
  }
  if (aLeft->EndPoint().Distance (aRight->StartPoint()) > NvGeContext::gTol.equalPoint())
  {
    throw NvException ("NvGeNurbCurve2d::joinWith(): the spline ends do not meet");
  }
  ...
  {
    throw NvException ("NvGeNurbCurve2d::joinWith(): clamped splines are required");
  }
  const int aDegree = std::max (aDefL.Degree, aDefR.Degree);
  if (aDefL.Degree < aDegree)
  {
    occ::handle<NvGeNurb2dData> aLeftElev = PrivateCopy (aLeft);
    try
    {
...
NvGeNurbCurve2d& NvGeNurbCurve2d::hardTrimByParams (double theNewStartParam, double theNewEndParam)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (theNewEndParam - theNewStartParam <= 0.0)
  {
    throw NvException ("NvGeNurbCurve2d::hardTrimByParams(): the end parameter must be greater"
                       " than the start parameter");
  }
  if (theNewStartParam < aSpline->FirstParameter() || theNewEndParam > aSpline->LastParameter())
  {
    throw NvException ("NvGeNurbCurve2d::hardTrimByParams(): the parameters are outside the"
                       " curve interval");
  }
  occ::handle<NvGeNurb2dData> aData = PrivateCopy (aSpline);
  ...
  aData->Fit = NvGeFitData2d();   // fit parameters no longer match the trimmed curve
...
NvGeNurbCurve2d& NvGeNurbCurve2d::makeRational (double theWeight)
{
  if (theWeight <= 0.0)
  {
    throw NvException ("NvGeNurbCurve2d::makeRational(): the weight must be positive");
  }
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (aSpline->Weights() != nullptr)
  {
  ...
  occ::handle<NvGeNurb2dData> aData = DataFromDefinition (aDef, "makeRational");
  const NvGeNurb2dData* aSrc = FitDataOf (aSpline);
  if (aSrc != nullptr)
  {
    aData->Fit = aSrc->Fit;
...
NvGeNurbCurve2d& NvGeNurbCurve2d::makeClosed ()
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (aSpline->IsPeriodic()
...
NvGeNurbCurve2d& NvGeNurbCurve2d::makePeriodic ()
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (aSpline->IsPeriodic())
  {
  ...
  occ::handle<NvGeNurb2dData> aData = PrivateCopy (aSpline);
  occ::handle<NvGeNurb2dData> aClosed = ClosedCopy (*aData, "makePeriodic");
  try
  {
    aClosed->SetPeriodic();
  }
...
NvGeNurbCurve2d& NvGeNurbCurve2d::makeNonPeriodic ()
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (!aSpline->IsPeriodic())
  {
...
NvGeNurbCurve2d& NvGeNurbCurve2d::makeOpen ()
{
  occ::handle<NvGeNurb2dData> aData = PrivateCopy (SplineOf (mpImpEnt));
  if (aData->IsPeriodic())
  {
  ...
  occ::handle<NvGeNurb2dData> aRes = DataFromDefinition (aDef, "makeOpen");
  aRes->Fit = NvGeFitData2d();   // fit parameters spanned the removed wrap span
...
NvGeNurbCurve2d& NvGeNurbCurve2d::elevateDegree (int thePlusDegree)
{
  if (thePlusDegree < 0)
  {
    throw NvException ("NvGeNurbCurve2d::elevateDegree(): the degree increment must not be negative");
  }
  if (thePlusDegree == 0)
  {
    return *this;
  ...
  if (aTarget > Geom2d_BSplineCurve::MaxDegree())
  {
    throw NvException ("NvGeNurbCurve2d::elevateDegree(): the resulting degree is out of range");
  }
  occ::handle<NvGeNurb2dData> aData = PrivateCopy (aSpline);
  try
  {
...
Adesk::Boolean NvGeNurbCurve2d::addControlPointAt (double theNewKnot, const NvGePoint2d& thePoint,
                                                   double theWeight)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  ...
  try
  {
    // Multiplicity 1 keeps the inserted knot regular so that the matching
    // pole removal undoes the insertion exactly.
    aData->InsertKnot (theNewKnot, 1, aKnotTol);
  }
  ...
  try
  {
    ...
    {
      aData->SetPole (aPoleIdx, gp_Pnt2d (thePoint.x, thePoint.y));
    }
    else
    {
      aData->SetPole (aPoleIdx, gp_Pnt2d (thePoint.x, thePoint.y), theWeight);
    }
  }
  catch (const Standard_Failure&)
  {
...
Adesk::Boolean NvGeNurbCurve2d::deleteControlPointAt (int theIndex)
{
  const occ::handle<Geom2d_BSplineCurve> aSpline = SplineOf (mpImpEnt);
  if (aSpline->IsPeriodic()
  ...
  occ::handle<NvGeNurb2dData> aData = DataFromDefinition (aDef, "deleteControlPointAt");
  const NvGeNurb2dData* aSrc = FitDataOf (aSpline);
  if (aSrc != nullptr)
  {
    aData->Fit = aSrc->Fit;
...
NvGeNurbCurve2d& NvGeNurbCurve2d::operator = (const NvGeNurbCurve2d& theSpline)
{
