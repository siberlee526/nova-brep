...
#include "NvAChar.h"            // NCHAR typedef

#define Nova_Boolean_is_bool 1
#define Nova_Int32_is_int 1


#pragma pack (push, 8)

#if defined(_MSC_VER)
#ifndef _NOVA_CROSS_PLATFORM_
    #define _NOVA_WINDOWS_ 1
#endif
#elif defined(__clang__)
#if defined(_NOVA_MAC_)
    #define __w64
#endif
...
struct Nova
{
    ...
    //
    // We also need to be sure to use the Nova::Int32 and Nova::UInt32
    ...
    // fix is to consistently use Nova::Int32 and Nova::UInt32 in place
    ...
    // Todo: consider removing these from nova.h
    //
...
#if !defined(_WIN64) && !defined (_NV64)
...
// Use NOVA_NO_VTABLE on base classes which:
...
//
#if defined(_MSC_VER)
    #define NOVA_NO_VTABLE           __declspec(novtable)
    #define NOVA_STDCALL             __stdcall
    #define NOVA_DEPRECATED          __declspec(deprecated)
    #define NOVA_DEPRECATED_MSG(MSG) __declspec(deprecated(MSG))
    #define NOVA_DATA_IMPORT         __declspec(dllimport)
    #define NOVA_PACK_BEGIN __pragma(pack(push,1))
    #define NOVA_PACK_END __pragma(pack(pop))
    #define NOVA_HIDDEN_SYMBOL //symbols are hidden by default with MSVC toolchain
#else 
    #define NOVA_NO_VTABLE
    ...
    // from the compiler mentioning that it isn't supported.
        ...
        #define NOVA_STDCALL
    #else
        #define NOVA_STDCALL         __attribute__((stdcall))   
    #endif // __LP64__
    
    #define NOVA_DEPRECATED          __attribute__((__deprecated__))
    #define NOVA_DEPRECATED_MSG(MSG) __attribute__((__deprecated__))
    #define NOVA_DATA_IMPORT         extern
    ...
    #define NOVA_PACK_BEGIN
    #define NOVA_PACK_END              __attribute__ ((packed))
    #define NOVA_HIDDEN_SYMBOL         __attribute__ ((visibility("hidden")))
#endif //_MSC_VER

#if defined(_MSC_EXTENSIONS) || defined (__clang__)
#define NOVA_OVERRIDE  override
    #if defined(_MSC_VER) && (_MSC_VER <= 1600) //VS2010 and earlier
        #define NOVA_SEALED sealed
    #else
#define NOVA_SEALED final
    #endif
#else //_MSC_EXTENSIONS
    #define NOVA_OVERRIDE
    #define NOVA_SEALED 
...
#define MIGRATION_ERRORS
    ...
    #define NOVA_SEALED_VIRTUAL virtual
    #if !defined(NOVA_SEALED)
        #define NOVA_SEALED sealed
    #endif //NOVA_SEALED
#else //MIGRATION_ERRORS
    #define NOVA_SEALED_VIRTUAL 
    #if !defined(NOVA_SEALED)
        #define NOVA_SEALED sealed
    #endif //NOVA_SEALED
...
#ifdef _MSC_VER
#define NOVA_UNREFED_PARAM(x) x
#elif defined(__clang__)
#define NOVA_UNREFED_PARAM(x) (void)(x);
#else
#error Unknown compiler.
#endif

#ifdef _MSC_VER
#define NOVA_UNREACHABLE __assume(false)
#elif defined(__clang__)
...
#define NOVA_FORCE_OPTNONE
     ...
     **/
    #if defined(__ANDROID__) || defined(NOVA_APPLE_WITH_CLANG_8_1_OR_HIGHER)
        #define NOVA_FORCE_OPTNONE __attribute__((optnone))
    #else
        #define NOVA_FORCE_OPTNONE
    #endif
#else
...
// __declspec(dllexport). Define NOVA_EXPORT to use the relevant
// export macro based on the current platform.
#if defined(_NOVA_WEB_)
    /*
    ...
    */
    ...
    // #define NOVA_EXPORT EMSCRIPTEN_KEEPALIVE
    #define NOVA_EXPORT 
#else
...
// The NOVA_NODISCARD macro is a convenience to check
...
#define NOVA_DEPRECATE_FOR_INTERNAL_USE NOVA_DEPRECATED
#else
#define NOVA_DEPRECATE_FOR_INTERNAL_USE
#endif

// Suppresses the deprecated API warning for the next line
#define NOVA_SUPPRESS_DEPRECATED _Pragma("warning(suppress:4996)")
